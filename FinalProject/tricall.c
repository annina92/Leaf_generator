...
int* getTriangleIndices(float* points, int dim, int* numIndices)
{
	//gli passo un array points di Point2D di dimensione dim
	struct triangulateio in, mid, out, vorout;

	/* Define input points. */
	in.numberofpoints = dim; //dim
	...
	triangulate("pczAvn", &in, &mid, &vorout);
	...
	triangulate("pcazBPX", &mid, &out, (struct triangulateio *) NULL);
	...
	indices = (unsigned short *)calloc(out.numberoftriangles * 3, sizeof(unsigned short));
	for (int i = 0; i < out.numberoftriangles*3; i++) {
		indices[i]=  out.trianglelist[i];
